...
export default function PropertiesPage() {
  ...
  async function importGestionIsr(event: React.FormEvent<HTMLFormElement>) {
    ...
    setNotice(
      `Synchronisation terminee: ${payload.added} ajoutes, ${payload.updated} mis a jour, ${payload.removed} retires.`,
    );
    setLoading(false);
  ...
  return (
    <section className="grid gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-[family-name:var(--font-barlow-condensed)] text-4xl font-bold">Logements</h2>
          <p className="text-sm text-emerald-800">Gestion, recherche, import/export CSV et synchronisation Gestion ISR</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            className="rounded-lg border border-emerald-300 bg-white px-4 py-3 text-sm font-semibold"
            onClick={() => {
      ...
      <section className="card grid gap-3 border-2 border-emerald-400 p-5">
        <div>
          ...
          <p className="text-sm text-emerald-800">
            Utilise ce bouton pour ajouter les nouveaux logements Gestion ISR et retirer ceux qui ne sont plus affiches.
          </p>
        </div>
        ...
        <form onSubmit={importGestionIsr} className="grid gap-3 md:grid-cols-[1fr_auto]">
          <input
            name="url"
            placeholder="URL page logements Gestion ISR (optionnel si GESTION_ISR_LISTINGS_URL est configure)"
      ...
      <form onSubmit={createProperty} className="card grid gap-3 p-4 md:grid-cols-2">
        ...
        <button className="rounded-lg bg-[var(--accent)] px-4 py-3 text-white md:col-span-2" disabled={loading}>
          {loading ? "Enregistrement..." : "Ajouter le logement"}
        </button>
      </form>
      ...
      <div className="grid gap-3">
        {items.map((item) => (
          <article key={item.id} className="card grid gap-3 p-4">
            {item.photos?.[0] ? (
              <div className="overflow-hidden rounded-xl border border-emerald-100 bg-emerald-50">
                ...
                <img
                  ...
                  alt={item.photos[0].description || `${item.address}`}
                  className="h-56 w-full object-cover"
                />
              </div>
            ) : (
              <div className="flex h-40 items-center justify-center rounded-xl border border-dashed border-emerald-200 bg-emerald-50 text-sm text-emerald-700">
                Aucune photo visible pour ce logement
              </div>
            )}
            ...
            <div className="flex flex-wrap gap-2">
              <a
                href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${item.address}, ${item.city}`)}`}
                target="_blank"
                rel="noreferrer"
              ...
              <button type="button" onClick={() => generateAds(item.id)} className="rounded-lg border border-emerald-200 px-3 py-2 text-sm">
                Generer annonces
              </button>
              <a className="rounded-lg border border-emerald-200 px-3 py-2 text-sm" href={`/visits?propertyId=${item.id}`}>
                Planifier une visite
              </a>
              {item.gestionIsrUrl ? (
                <a href={item.gestionIsrUrl} target="_blank" rel="noreferrer" className="rounded-lg border border-emerald-200 px-3 py-2 text-sm">
                  Ouvrir la fiche ISR
                </a>
              ) : null}
