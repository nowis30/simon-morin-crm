...
async function getCatalogItems() {
  ...
  if (!response.ok) {
    return [] as Array<{
      ...
      features: string[];
      building?: { name?: string | null; address: string; city: string; district?: string | null } | null;
      rentalUnit?: { unitNumber?: string | null; floor?: string | null } | null;
    }>;
  }
    ...
    <main className="min-h-screen bg-slate-950 px-6 py-16 text-slate-100">
      <div className="mx-auto flex max-w-6xl flex-col gap-8">
        <div className="space-y-3">
          <p className="text-sm uppercase tracking-[0.3em] text-slate-400">Catalogue public</p>
          <h1 className="text-4xl font-semibold">Découvrez les logements disponibles</h1>
          <p className="max-w-2xl text-slate-300">Une première vitrine publique, alimentée par la synchronisation Gestion ISR et prête pour une conversion vers les visites et les prospects.</p>
        </div>
            ...
            <article key={item.id} className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/70 shadow-lg">
              ...
              <div className="space-y-3 p-5">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm font-medium text-emerald-400">{item.status}</p>
                  <p className="text-sm text-slate-400">{item.codeIsr}</p>
                </div>
                <div>
