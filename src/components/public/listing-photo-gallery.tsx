      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3">
        ...
        >
          <img
            ...
            className="h-72 w-full object-cover transition duration-300 sm:h-80 lg:h-96"
            loading="lazy"
            onLoad={() => setIsLoaded(true)}
          />
          {!isLoaded ? <div className="absolute inset-0 animate-pulse bg-slate-800" /> : null}
          <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-slate-950/90 to-transparent px-3 py-3 text-xs text-slate-100">
            <span>{title}</span>
          ...
          <button
            ...
            className="absolute left-3 top-1/2 -translate-y-1/2 rounded-full border border-white/20 bg-slate-900/70 px-3 py-2 text-sm text-white"
          >
            ←
          </button>
          <button
            type="button"
            onClick={() => goTo(activeIndex + 1)}
            className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full border border-white/20 bg-slate-900/70 px-3 py-2 text-sm text-white"
          >
            →
          </button>
          <button
            ...
            className="absolute right-3 top-3 rounded-full border border-white/20 bg-slate-900/70 px-3 py-2 text-xs text-white"
          >
            Plein écran
          </button>
        </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {allPhotos.map((photo, index) => (
          <button
            ...
            className={`h-16 w-20 flex-shrink-0 overflow-hidden rounded-lg border ${activeIndex === index ? "border-emerald-400" : "border-slate-700"}`}
          >
            <img src={photo.url} alt={photo.description || `${title} ${index + 1}`} className="h-full w-full object-cover" loading="lazy" />
          </button>
        ))}
      </div>
